...
export function createThreeScene(container: HTMLElement) {
  ...
  const planeMaterial = new THREE.MeshBasicMaterial({ color: 0xfafafa, side: THREE.DoubleSide });
  const plane = new THREE.Mesh(planeGeometry, planeMaterial);
  scene.add(plane);
  plane.rotation.x = -0.5 * Math.PI; // Rotaciona o plano para ficar horizontal
  ...
  const ambienteLight = new THREE.AmbientLight(0x404040, 1); // Luz ambiente
